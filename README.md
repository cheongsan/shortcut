...
| 단계 | 할 일 |
|---|---|
| [1](#1-notion-표-만들기) | Notion 표 만들기 |
| [2](#2-notion-연결하기) | Notion 연결하고 값 3개 챙기기 |
| [3](#3-환경변수) | 환경변수 확인 |
| [4](#4-vercel-배포) | Vercel 배포 |
...
1. <https://www.notion.so/my-integrations> → **New integration** 을 만듭니다.
   권한은 **Read content** 를 선택합니다.

2. 생성 후 나오는 **시크릿**을 복사합니다. → 이 값이 **`NOTION_TOKEN`** 입니다.
   `ntn_` 으로 시작합니다 (예전에 만든 것은 `secret_`).

3. **1번에서 만든 Notion 표 페이지를 열고, `⋯` → `Connections` → 방금 만든 인테그레이션을
   추가합니다.**
...
2번에서 챙긴 값 두 개가 필수입니다. 나머지는 선택입니다.
...
3. **Environment Variables** 에 [3번](#3-환경변수)의 값을 넣습니다.
...
**Notion 표를 고치면 됩니다.** 재배포는 필요 없습니다.

다만 서비스가 Notion을 매번 읽지 않고 **5분 주기로 모아서 읽기** 때문에 즉시 반영되지는 않습니다.
...
| 속성 이름·타입 변경 | 이름을 되돌리거나 코드 수정 필요 → [6. 문제 해결](#6-문제-해결) |

### 바로가기 중지

줄을 지우지 말고 `Enabled` 를 **`Disabled`** 로 바꿉니다. 그동안 그 키는
...
대개 [2번 3단계](#2-notion-연결하기)(표에 인테그레이션 연결)를 빠뜨린 경우입니다.
...
목적지가 미리보기 이미지를 제공하지 않거나 외부 사용을 막아둔 경우이며,
...
`site.config.ts` 를 고치고 GitHub에 올리면, Vercel이 자동으로 다시 배포합니다.
...
2. Notion 값 채우기
   ...
   [2번](#2-notion-연결하기)에서 챙긴 `NOTION_TOKEN` 과 `NOTION_DATABASE_ID` 를 넣습니다.
   **값이 없으면 실행되지 않습니다.**
