...
> **3번이 가장 많이 놓치는 단계입니다.** 인테그레이션을 만들기만 하면 아무 표도 읽지 못하고,
> **표마다 따로 연결해야** 합니다. 빠뜨리면 Notion이 "그런 데이터베이스가 없다"고 답하는데
> 실제로는 권한 문제라 헷갈립니다. → [7. 문제 해결](#7-문제-해결)
...
> **`NOTION_TOKEN` 은 표를 읽을 수 있는 열쇠입니다.** Vercel 환경변수 화면에만 넣어 주세요.
> 저장소에 올리면 안 됩니다. 로컬에서는 `.env.local` 에 넣으면 되고, 이 파일은 이미
> Git에서 제외돼 있습니다.

<details>
<summary><code>NOTION_DATA_SOURCE_ID</code> 를 채우고 싶다면</summary>

비워두면 서비스가 시작할 때 알아서 찾습니다. 다만 그때마다 Notion에 한 번 더 물어보므로,
직접 넣어두면 첫 응답이 조금 빨라집니다.

값은 [9번 항목](#9-내-컴퓨터에서-실행해-보기)의 준비를 마친 뒤 아래 명령으로 확인합니다.

```bash
npm run notion:datasource
```

표에 데이터 소스가 여러 개인 경우에는 **직접 지정하는 편이 안전합니다.** 비워두면 그중
어느 것이 선택될지 보장되지 않습니다.
...
> **Preview 배포 보호를 권합니다.** Vercel의 브랜치 미리보기 주소(`*.vercel.app`)는
> 누구나 접속할 수 있고 같은 환경변수를 씁니다. 프로덕션 도메인에만 접근 제한을 걸면
> 이 주소로 우회되므로, **Settings** → **Deployment Protection** 에서 Preview에도 켜 주세요.
...
> 읽어둔 내용은 배포와 별개로 보관됩니다. 확실한 방법은 5분쯤 기다리는 것입니다.
>
> 저장하는 즉시 반영되게 하고 싶으시면 알려주세요 — 지금은 쓰지 않는 기능을 열어두지
> 않으려고 일부러 넣지 않았습니다.
...
캐싱 방식, 미리보기 이미지를 서버가 대신 받지 않는 이유, 보안상 고려한 점, 블로그 디자인을
옮기며 일부러 바꾼 부분은 [`docs/DESIGN.md`](docs/DESIGN.md) 에 정리해 두었습니다.
