...
**Notion 자격증명이 없으면 실패합니다.** 예시 데이터로 대체하는 경로는 없습니다 — 가짜
바로가기를 서빙하면 잘못된 배포가 "동작하는 것처럼 보이는 사이트" 뒤에 숨습니다.
`.env.local`에 `NOTION_TOKEN`과 `NOTION_DATABASE_ID`를 넣어야 개발 서버가 뜹니다.

실패 지점은 두 갈래로 구분됩니다:

| 상황 | 동작 |
|---|---|
| 환경변수 누락 | `MissingNotionConfigError` — **빌드가 실패**합니다 (`listAllKeys`가 rethrow) |
| Notion API 장애 | 경고만 남기고 온디맨드 렌더로 내려갑니다 (배포를 막지 않음) |

`unstable_cache` 엔트리는 `.next/cache`에 남아 **빌드를 넘어 재사용됩니다.** 그래서
자격증명 없이도 이전 빌드의 캐시가 있으면 빌드가 통과할 수 있습니다. 실패를 확인하려면
`rm -rf .next` 후 빌드하세요.

### OG 파서를 로컬에서 테스트하기

[lib/url.ts](lib/url.ts)의 가드가 루프백을 막기 때문에 로컬 테스트 서버(`127.0.0.1`)를 향한
...
300초 창 안에서는 같은 데이터가 나온다 — 재배포는 반영을 앞당기는 수단이 아니다.
